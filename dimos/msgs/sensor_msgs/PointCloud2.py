...
class PointCloud2(Timestamped):
    ...
    def __getstate__(self) -> dict[str, object]:
        ...
        # ``functools.cached_property`` memoises its value under the attribute's
        # own name in ``__dict__``. Several of these (e.g. the Open3D bbox
        # geometries) are pybind11 C++ objects that cannot be pickled and would
        # fail with "cannot pickle 'open3d.cpu.pybind.geometry.*' object". Strip
        # every cached value here; they are recomputed lazily after load.
